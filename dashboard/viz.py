...
def ranking_bar(ranking: pd.DataFrame, metric: str = "phci",
                title: str = "Peak-Hour Congestion Index — 25 corridors",
    ...
    The full-page chart packs 25 rows and uses the default 55; the compact
...
def mini_map(display: pd.DataFrame, top_n: int = 5,
    ...
    All 25 corridors are drawn faintly so the geographic context is preserved;
    ...
    if display.empty:
        return pdk.Deck(layers=[], initial_view_state=pdk.ViewState(
            longitude=76.78, latitude=30.74, zoom=11.5,
        ))
    df = display.copy()
    ...
    return pdk.Deck(
        ...
        initial_view_state=pdk.ViewState(
            longitude=76.78, latitude=30.74, zoom=11.5, pitch=0, bearing=0,
        ),
